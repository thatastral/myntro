...
- **Music blocks** (`type: 'music'`, `components/blocks/MusicBlock.tsx`): replaces the old Spotify-only block. Auto-detects platform from the pasted URL and renders the correct embed for Spotify, Apple Music, and YouTube Music. Legacy `type: 'spotify'` DB rows are handled by the same renderer. New blocks are saved as `type: 'music'`.
- **Text blocks** (`type: 'text'`, `components/blocks/TextBlock.tsx`): rich-text block with bold, italic, font size (sm/md/lg), alignment (left/center/right), and font family (DM Sans, Funnel Display, Mono). Content stored as flat string fields in `content` JSONB. Click to edit via modal (same pattern as NoteBlock). Migration: `012_text_block_type.sql`.
- **Image blocks** — upgraded: height resize via drag handle at bottom (stored as `content.height` px string, default 200, range 100–480). Width toggle (1→2 col) via `onUpdateSpan` prop. Caption rendered as bottom overlay gradient (`#0F1702` → transparent). Clicking the image on the edit page activates inline edit mode. `updateBlockSpan` in `useProfile` patches `span` directly.
